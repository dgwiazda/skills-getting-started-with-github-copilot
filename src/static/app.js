document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear loading message and previous activity options
               ...
               .map(
                 (participant) => `<li class="participant-item">
                     ...
                     <button type="button" class="participant-remove" data-activity="${encodeURIComponent(
                       name
                     )}" data-email="${encodeURIComponent(participant)}" title="Remove participant">&times;</button>
                   </li>`
               )
               .join("")}</ul>`
          : `<p class="info">No participants are signed up yet.</p>`;
